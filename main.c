...
/* Feature added for extra credits */
...
struct subscriberStruct
{
	pid32 pid;
	topic16 group;
	void (*handlerPointer)(topic16, void*, uint32);
...
process Broker()
{
	...
	while(1)
	{
		...
		while(subscriberEntry != (struct subscriberStruct *) NULL)
		{
			...
			subscriberEntry -> handlerPointer(brokerEntry -> topic, brokerEntry -> data, brokerEntry -> size);
			subscriberEntry = subscriberEntry -> next;
		}
		signal(topicTableSem);
